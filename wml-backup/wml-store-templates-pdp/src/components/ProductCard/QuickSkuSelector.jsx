...
export default function QuickSkuSelector(props) {
	...
	useEffect(() => {
		...
		const selectedVariations = product.items.reduce((acc, item) => {
			...
			item.variations.forEach((variation) => {
				...
				let existingVariation = acc.find((variationItem) => variationItem.name === name)
				...
				const valueExists = existingVariation.values.some((value) => value.name === values[0])
				if (!valueExists) {
	...
	const isSkuOptionAvailable = (skuName, valueName) => {
		if (!hasInteracted || !currentSku?.variations) {
			const variation = skuVariations?.find((variationItem) => variationItem.name === skuName)
			const value = variation?.values?.find((variationValue) => variationValue.name === valueName)
							...
							{sortSku([...(sku?.values || [])])?.map((value) => {
								...
								return (
									<View
										...
										<View
											data-unavailable={!isAvailable ? '' : undefined}>
											<Text className='data-[unavailable]:text-[#D2C9C0]'>
												{value.name}
											</Text>
										</View>
									</View>
