...
export default function ShelfOfProductsCarousel(props) {
	...
	if (isLoading) {
		return (
			<View className={`flex overflow-x-auto overflow-y-hidden ${HIDE_SCROLLBAR_CLASS}`} style={HIDE_SCROLLBAR_STYLE}>
	...
	return (
		<>
			<View
				id={CAROUSEL_ID}
				className={`flex overflow-x-auto overflow-y-hidden ${HIDE_SCROLLBAR_CLASS}`}
				style={{
			...
			{/* Dots */}
			{products?.length > 1 && (
				<View className='mt-4 flex justify-center gap-1.5'>
