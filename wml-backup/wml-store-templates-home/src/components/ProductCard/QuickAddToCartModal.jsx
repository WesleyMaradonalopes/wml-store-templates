...
import { isSkuAvailable, Loading } from 'wml-store-templates-shared'

import CustomModal from '../CustomModal/CustomModal'
import QuickSkuSelector from './QuickSkuSelector'

export default function QuickAddToCartModal(props) {
	const { open, onClose, product, currentSku, onSkuChange, onConfirm, loading, isLoadingProduct = false } = props
					...
					<View className='mt-8 px-[34px] pb-[16px]'>
						<Button
							className={`w-full rounded-pill transition-all duration-300 ${
								isLoadingProduct || !userHasSelected || loading || !isCurrentSkuAvailable
