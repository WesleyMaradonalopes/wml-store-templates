...
export default function CustomInput(props) {
	...
	return (
		<View className='w-full'>
			{label && (
				<View className={checkoutVariant ? 'mb-[4px]' : 'mb-1'}>
					<Text className={checkoutVariant ? 'text-[12px] font-normal leading-[150%] text-[#0F0805]' : 'text-xs font-bold'}>{label}</Text>
				</View>
			)}
			<View className='relative'>
				<TextInput
					className={checkoutVariant
						? `h-[40px] w-full rounded-[8px] border border-solid bg-transparent px-3 py-2 text-[16px] font-normal leading-[150%] text-[#0F0805] placeholder:text-[16px] placeholder:font-normal placeholder:leading-[150%] placeholder:text-[#575756] focus:outline-none ${error ? 'border-[#D92D20]' : 'border-[#B0A69B]'} ${className || ''}`
						: `w-full rounded border-2 border-solid focus:outline-none ${error ? 'border-[#D92D20]' : 'border-gray-300'} ${className || ''}`}
