...
import { CustomerOrder, CustomerOrderBundleItem, CustomerOrderItem, getCustomerOrder, orderStatusLabel } from '@/services/orders';
...
function orderBundlePrice(item: CustomerOrderBundleItem) {
  return item.price ?? item.priceDefinition?.total;
}

function OrderItemRow({ item, index }: { item: CustomerOrderItem; index: number }) {
  return <View key={`${item.name}-${index}`}>
    <View style={styles.item}>{item.imageUrl ? <Image source={{ uri: item.imageUrl }} style={styles.image} /> : <View style={[styles.image, styles.placeholder]} />}<View style={styles.itemInfo}><OrderText numberOfLines={3}>{item.name || 'Produto'}</OrderText><OrderText secondary>{item.quantity ?? 1} un · {money(item.price)}</OrderText></View></View>
    {(item.bundleItems ?? []).map((bundleItem, bundleIndex) => <View key={`${bundleItem.name}-${bundleItem.id ?? bundleIndex}`} style={styles.serviceItem}><View style={[styles.serviceImage, styles.placeholder]} /><View style={styles.itemInfo}><OrderText numberOfLines={3}>{bundleItem.name || 'Serviço adicional'}</OrderText><OrderText secondary>SKU {bundleItem.id || '—'} · {bundleItem.quantity ?? 1} un · {money(orderBundlePrice(bundleItem))}</OrderText></View></View>)}
  </View>;
}
