...
export function ScreenHeader({ back = true, title, onBack, showSearch = true, showCart = true }: ScreenHeaderProps) {
  const router = useRouter();
  const goBack = onBack ?? (() => router.back());
  return (
    <View style={styles.header}>
      {back ? <Pressable onPress={goBack} style={styles.side}><ArrowLeftIAIcon color="#231f20" size={21} /></Pressable> : <View style={styles.side} />}
      <View pointerEvents="box-none" style={styles.center}>{title ? <ThemedTitle>{title}</ThemedTitle> : <Pressable accessibilityLabel="Ir para o início" onPress={() => router.replace('/')} style={styles.logoButton}><HopeLogoIcon color="#231f20" width={76} height={20} /></Pressable>}</View>
